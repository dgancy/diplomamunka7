...
export default function MasterTheoramAssignment() {
  ...
  function Check() {
    ...
    console.log(differencial + " : " + nlog);

    var eset;
    if (a_elem >= 1 && b_elem > 1) {
      if (a_elem === b_elem && parseFloat(differencial) === parseFloat(nlog)) {
        eset = 2;
      }
      if (parseFloat(differencial) < parseFloat(nlog)) {
        eset = 1;
        epszilon = nlog - differencial;
      }
      if (parseFloat(differencial) > parseFloat(nlog)) {
        eset = 3;
        epszilon = differencial - nlog;
      }
    }
    ...
    document.getElementById("final-4").innerHTML = mistakes[4];
  }
